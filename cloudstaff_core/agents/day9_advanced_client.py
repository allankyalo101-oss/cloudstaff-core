...
def client_report(name):
    ...
    avg_invoice = total_invoiced / max(1, get_invoice_count(name))
    return {
        "name": name,
        ...
        "invoice_count": get_invoice_count(name)
    }

def get_invoice_count(name):
    conn = connect_db()
    cursor = conn.cursor()
...
def notify_overdue():
    ...
    for entry in overdue:
        print(f"FOLLOW-UP: {entry['client']} has overdue balance of {entry['amount']}")

def notify_priority():
    high_value = high_value_clients()
    for entry in high_value:
        print(f"PRIORITY ALERT: {entry['client']} is high-value with total invoiced {entry['total_invoiced']}")

if __name__ == "__main__":
    print("Day 9 multi-client system loaded.")
